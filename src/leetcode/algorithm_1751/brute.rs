...
impl Solution {
    pub fn max_value(events: Vec<Vec<i32>>, k: i32) -> i32 {
        ...
        Self::optimal_of_day(&indexed, 0, last_start, k)
    }

    fn optimal_of_day(events: &HashMap<i32, Vec<Vec<i32>>>, day: i32, last: i32, k: i32) -> i32 {
        ...
        if let Some(today) = events.get(&day) {
            for e in today.iter() {
                let value = e[2] + Self::optimal_of_day(events, e[1] + 1, last, k - 1);
                if value > optimal {
                    optimal = value;
                }
            }
        ...
        let no_attend_any = Self::optimal_of_day(events, day + 1, last, k);
